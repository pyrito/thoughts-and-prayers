...
class UpdateThread(Thread):
    ...
    def updater(self):
        ...
        while not thread_stop_event.isSet():
            ...
            if len(event) == 0:
                db.session.execute("INSERT INTO events VALUES(:id, :location, :count, :when, :last, :initial, :past)", {'id': 0, 'location': "Noblesville", 'count': 0, 'when': datetime.datetime(2018, 5, 25), 'last': 0, 'initial': 0, 'past': True})
                db.commit()
                event = db.session.execute("""SELECT count FROM events WHERE place = :location """, {'location':'Noblesville'}).fetchone()
            num = event[0]
